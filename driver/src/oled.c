...
**********************************************/
/**************************************************************
在产生该信号前，IIC总线要处于空闲状态，即同时处于高电平
//IIC Start在时钟线SCL保持高电平期间，数据线SDA上的电平被拉低
***************************************************************/
void IIC2_Start()
{
...
/*****************************************************************
//IIC Stop在SCL保持高电平期间，SDA被释放，使得回到高电平为停止信号
******************************************************************/
void IIC2_Stop()
{
  OLED_SCLK_Set();
...
/**********************************************
// IIC Write byte按位写入   先写入最高位，然后左移写次高位
**********************************************/
...
void oled_region(u8 x1,u8 y1,u8 x2,u8 y2)
{
	u8 i,n;		    
	for(i=y1;i<y2;i++)  
	{  
...
//x：横坐标，y：纵坐标，no：字模位置，i：字数
void OLED_ShowCHinese2(u8 x,u8 y,u8 no,u8 i)
{      			    
	u8 t=0;
  u8 a=x;
	u8 b=y;
